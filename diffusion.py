...
def rgb2normalmap_batched(normal_maps):
    normal_maps = normal_maps[:, :,:,0,:3].numpy()
    min_value = np.min(normal_maps, axis=(1, 2, 3), keepdims=True)
    max_value = np.max(normal_maps, axis=(1, 2, 3), keepdims=True)
    normalized_normal_maps = np.where(normal_maps != 0, (normal_maps - min_value) / (max_value - min_value), 0)
    normal_map_image = (normalized_normal_maps * 255).astype(np.uint8)
    detected_map = HWC4(normal_map_image)
    return detected_map
...
def process_depth_map(depth):
    if depth.ndim == 3:
        ...
        # Batched processing
        max_depth = depth.view(len(depth), -1).max(dim=1)[0]
        indices = depth == -1
        depth = max_depth[:, None, None, None] - depth
        depth[indices] = 0
        max_depth = depth.view(len(depth), -1).max(dim=1)[0]
        depth = depth / max_depth[:, None, None, None]

        totdepth = [transform(depth[i]) for i in range(len(depth))]
        return totdepth
...
def run_diffusion_batched(
    ...
    depth_map = process_depth_map(depth_map)
    if normal_map_input is not None:
        normal_map_input = rgb2normalmap_batched(normal_map_input)
        normal_map = [Image.fromarray(normal_map_input[i]) for i in range(len(normal_map_input))]

        # Zip the depth map and normal map
        control_image = [list(maps) for maps in zip(depth_map, normal_map)]
    else:
        control_image = [depth_map]

    generator = torch.manual_seed(60)
    pos_prompt = [f"{prompt},best quality,highly detailed,photorealistic,photo"] * len(control_image)
    negative_prompt = ["lowres,low quality,monochrome,watermark"] * len(control_image)
    output_type = "pil"
    if use_latent:
        output_type = "latent"
    output = pipe(
